	...
	<body>
		...
		<div class="main">
			<div class="content">
				...
				<input type="button" class="button" value="Логи" onclick="window.location.href='logs.php';"/>
				...
				Административная панель служит для создания, редактирования и удаления записей на сайте.
					...
					<?php  
						$Sql = "SELECT * FROM `session` WHERE `IdUser` = ? ORDER BY `DateStart` DESC"; 
						...
						if($Query->num_rows > 0) {  
							...
							$TimeNow = time();
							$TimeDeltaMinutes = round(($TimeNow - $TimeEnd) / 60);  
							
							echo "<br>Последняя активная сессия была: {$TimeDeltaMinutes} минут назад";
						}
					?>
